...
if(NOT RISCV_TOOLCHAIN_PATH)
    if(DEFINED ENV{RISCV_TOOLCHAIN_PATH})
        ...
        if(NOT CMAKE_C_COMPILER)
            set(RISCV_TOOLCHAIN_PATH "C:/WORK/xpack-riscv-none-elf-gcc-15.2.0-1")
...
function(mdr32_print_size_of_target TARGET)
...
function(_mdr32_generate_file TARGET OUTPUT_EXTENSION OBJCOPY_BFD_OUTPUT)
...
function(mdr32_generate_binary_file TARGET)
    _mdr32_generate_file(${TARGET} "bin" "binary")
endfunction()

# This function adds post-build generation of the Motorola S-record file from the target ELF.
# The generated file will be placed in the same directory as the ELF file.
function(mdr32_generate_srec_file TARGET)
    _mdr32_generate_file(${TARGET} "srec" "srec")
endfunction()

# This function adds post-build generation of the Intel hex file from the target ELF.
# The generated file will be placed in the same directory as the ELF file.
function(mdr32_generate_hex_file TARGET)
    _mdr32_generate_file(${TARGET} "hex" "ihex")
endfunction()

#Generates binary file and copies it with name <TARGET>.elf.bin
function(mdr32_generate_elf_bin TARGET)
    mdr32_generate_binary_file(${TARGET})
...
function(mdr32_generate_elf_hex TARGET)
    mdr32_generate_hex_file(${TARGET})
...
function(mdr32_generate_lss_file TARGET)
...
if(NOT (TARGET MDR32::NoSys))
    add_library(MDR32::NoSys INTERFACE IMPORTED)
    target_compile_options(MDR32::NoSys INTERFACE $<$<C_COMPILER_ID:GNU>:--specs=nosys.specs>)
    target_link_options(MDR32::NoSys INTERFACE $<$<C_COMPILER_ID:GNU>:--specs=nosys.specs>)
endif()

if(NOT (TARGET MDR32::Nano))
    add_library(MDR32::Nano INTERFACE IMPORTED)
    target_compile_options(MDR32::Nano INTERFACE $<$<C_COMPILER_ID:GNU>:--specs=nano.specs>)
    target_link_options(MDR32::Nano INTERFACE $<$<C_COMPILER_ID:GNU>:--specs=nano.specs>)
endif()

if(NOT (TARGET MDR32::Nano::FloatPrint))
    add_library(MDR32::Nano::FloatPrint INTERFACE IMPORTED)
    target_link_options(MDR32::Nano::FloatPrint INTERFACE
        $<$<C_COMPILER_ID:GNU>:-Wl,--undefined,_printf_float>
    )
endif()

if(NOT (TARGET MDR32::Nano::FloatScan))
    add_library(MDR32::Nano::FloatScan INTERFACE IMPORTED)
    target_link_options(MDR32::Nano::FloatScan INTERFACE
